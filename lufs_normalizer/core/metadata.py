...
def _read_riff_chunks(filepath):
    """Read all chunks from a RIFF WAV file. Returns list of (id, data) tuples."""
    chunks = []
...
def _write_riff_file(filepath, chunks):
    """Write chunks back into a RIFF WAV file."""
    with open(filepath, 'wb') as f:
        # Placeholder for RIFF header
        f.write(b'RIFF')
        f.write(b'\x00\x00\x00\x00')  # Will be filled in
        f.write(b'WAVE')
...
def inject_bext_chunk(wav_path, metadata_dict):
    ...
    """
    try:
        chunks = _read_riff_chunks(wav_path)
        ...
        _write_riff_file(wav_path, new_chunks)
        return True
